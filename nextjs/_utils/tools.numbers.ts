...
export function formatAmount(
	...
	locales.push('en-US');
	locales.push(locale);
	let localMaximumFractionDigits = maximumFractionDigits;
	let localAmount = amount;
	if (localMaximumFractionDigits < minimumFractionDigits) {
		localMaximumFractionDigits = minimumFractionDigits;
	}
	if (!localAmount) {
		localAmount = 0;
	}
	...
	if (Number.isNaN(localAmount)) {
		localAmount = 0;
	}
	let formattedAmount = new Intl.NumberFormat(locales, {
		minimumFractionDigits,
		maximumFractionDigits: localMaximumFractionDigits
	}).format(localAmount);
...
function eToNumber(num: string): string {
	...
	const arr = localNumber.split(/[e]/gi);
	if (arr.length < 2) {
		return sign + localNumber;
	}
	const dot = '.';
	let n = arr[0];
	const exp = +arr[1];
